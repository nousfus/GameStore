...
public class MuaHang {
	...
	public String addToCart(Model m, @PathVariable("id")String id) {
		...
		if(a==null) {// Tạo Cart mới
			...
			CartItems cartitembyGame = cartitemsdao.findByGame(gamedao.findById(id).orElse(null));
			if(cartitembyGame!=null) {	// Nếu đã có game trong CartItems
				CartItems cartitem = new CartItems(cartitembyGame.getCart_item_id(),a.getCart_id(),game,cartitembyGame.getQuantity()+1); cartitemsdao.save(cartitem);
			}else {	
				String newcartitem_id = null;
				List<CartItems> abc = cartitemsdao.findAll();
				if(cartitemsdao.findAll().size()!=0) {
				...
				CartItems cartitem = new CartItems(newcartitem_id,a.getCart_id(),game,1); cartitemsdao.save(cartitem);
			}
		}
		return "forward:/list";
	}
